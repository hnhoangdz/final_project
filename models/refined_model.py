...
class SeparableConv2d(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=1, stride=1, padding=0, dilation=1, bias=False):
        ...
        self.depthwise = nn.Conv2d(in_channels, in_channels, kernel_size, stride=1, padding=padding, dilation=dilation, groups=in_channels,
                                   bias=bias)
        self.bnd = nn.BatchNorm2d(in_channels, affine=True, momentum=0.99, eps=1e-3)
        self.relu = nn.ReLU()
        self.pointwise = nn.Conv2d(in_channels, out_channels, 1, stride, 0, 1, 1, bias=bias)
        self.bnp = nn.BatchNorm2d(out_channels, affine=True, momentum=0.99, eps=1e-3)
...
class Block(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.sep_conv1 = conv3x3(in_channels, out_channels, kernel_size=3)
        self.sep_conv2 = conv3x3(out_channels, out_channels, kernel_size=3, stride=2)
